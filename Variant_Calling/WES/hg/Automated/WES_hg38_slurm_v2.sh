...
java -Djava.io.tmpdir=${input%_R1_001.fastq.gz}"_001_tmp" -jar ${PICARD} FastqToSam FASTQ=$input FASTQ2=${input%_R1_001.fastq.gz}"_R2_001.fastq.gz" OUTPUT=${input%_R1_001.fastq.gz}"_ubam.bam" READ_GROUP_NAME=UKB SAMPLE_NAME=$SampleID LIBRARY_NAME=`(echo $input| awk -F '[_]' '{print $(NF-2)}')` PLATFORM_UNIT=$platform_unit PLATFORM=ILLUMINA;
...
output6=${output5b%_step5b.bam}".g.vcf.gz"
...
java -jar ${GATK} HaplotypeCaller -R ${ref_path}"$reference" -I $output5b -O $output6 -ERC GVCF; 
...
bcftools norm -m -any -Oz -o ${output6%.g.vcf.gz}"_temp.vcf.gz" $output6
bcftools view -e 'ALT="<NON_REF>"' -v snps,indels -Oz -o ${output6%.g.vcf.gz}".vcf.gz" ${output6%.g.vcf.gz}"_temp.vcf.gz"
bcftools index -t ${output6%.g.vcf.gz}".vcf.gz"

gunzip ${output6%.g.vcf.gz}".vcf.gz"
...
$ref_path/annovar/table_annovar.pl ${output6%.g.vcf.gz}".vcf" $ref_path/annovar/humandb -buildver hg38 -out ${output6%.g.vcf.gz} -arg '-splicing 15',, -remove -protocol refGene,cytoBand,gnomad312_genome -operation g,r,f -nastring . -vcfinput ; 

output7=`ls $SampleID*multianno.txt`;
  if test -f "$output7"; then
   echo "$output7 exists" ;
   echo "Step 7 of Analysis Completed for $SampleID" ;
    mv `ls $SampleID*fastq.gz` Completed/
  else
...
rm ${SampleID}*step4*
rm ${output6%.g.vcf.gz}"_temp.vcf.gz"
